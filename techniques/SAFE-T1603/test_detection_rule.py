#!/usr/bin/env python3
"""
Test script for SAFE-T1603 detection rules
...
class DetectionRuleTester:
    """Test harness for SAFE-T1603 detection rules"""
    ...
    def print_results(self, results: Dict):
        ...
        print("=" * 80)
        print("SAFE-T1603 Detection Rule Test Results")
        print("=" * 80)
        ...
        for detail in results['details']:
            ...
            print(f"{status} [{detail['outcome']:4s}] {detail['test_id']:25s} - {detail['description']}")
        ...
        if results['false_positives'] > 0:
            ...
            print("   - Adjusting guard model confidence thresholds")
            print("   - Refining social engineering pattern detection")
            print("   - Adding more specific contextual conditions")

        if results['false_negatives'] > 0:
            print("⚠️  WARNING: False negatives detected. Consider:")
            print("   - Lowering semantic similarity thresholds")
            print("   - Adding more extraction phrase patterns")
            print("   - Implementing additional behavioral signals")

        if results['accuracy'] >= 0.90:
            print("✓ EXCELLENT: Detection accuracy is very high (≥90%)")
        elif results['accuracy'] >= 0.80:
            print("✓ GOOD: Detection accuracy is acceptable (≥80%)")
        else:
